...
const navigation = (() => {
    ...
    const navigationLink = menu.querySelectorAll('li a');
...
const formValidaton = (() =>{
	...
	const placeholderEffect_Active = (val,txt) =>{
        ...
        label[val].querySelector('span').innerText = txt.slice(1)
        label[val].style.fontSize = '12px'
	}
	const placeholderEffect_Reset = (val,txt) =>{
		label[val].style.top = '2.66em'
		label[val].style.color = 'gray'
        label[val].querySelector('span').innerText = txt
        label[val].style.fontSize = '13.4px'
	}
